...
Adafruit_SPITFT::Adafruit_SPITFT(uint16_t w, uint16_t h, SPI &spiClass,
  ...
  connection(TFT_HARD_SPI), _rst(rst), _cs(cs), _dc(dc), _spi(spiClass) {
}
...
void Adafruit_SPITFT::initSPI(uint32_t freq, uint8_t spiMode) {
    ...
    // // Set frequency
    // hwspi._spi.frequency(1000000);
    // hwspi._spi.format(8, spiMode); // Default to 8 bit mode
...
inline void Adafruit_SPITFT::SPI_BEGIN_TRANSACTION(void) {
    _spi.format(8, hwspi._mode);
    _spi.frequency(hwspi._freq);
}
